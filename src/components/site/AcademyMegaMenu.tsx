...
import { ChevronDown, ArrowRight, GraduationCap, MapPin, Sparkles, Briefcase, Users2, Building2, Award, HelpCircle, Mail, BookOpen } from "lucide-react";
import { PROGRAMS, CAMPUSES, CATEGORY_LABELS, type ProgramCategory } from "@/lib/academy-programs";
import { trackEvent } from "@/lib/analytics";
import { cn } from "@/lib/utils";

const CATEGORY_ORDER: ProgramCategory[] = ["ai", "data", "engineering", "cloud", "cyber", "product"];

const RESOURCES = [
  { to: "/academy/learning-paths", label: "Learning Paths", body: "Tracks by background and goal.", icon: Sparkles },
  { to: "/academy/placements", label: "Placements", body: "300+ hiring partners and support.", icon: Briefcase },
  { to: "/academy/scholarship", label: "Scholarship", body: "Merit and need-based awards.", icon: Award },
  { to: "/academy/corporate-training", label: "Corporate Training", body: "Upskilling for teams.", icon: Building2 },
  { to: "/academy/success-stories", label: "Success Stories", body: "Outcomes from our graduates.", icon: Users2 },
  { to: "/academy/certifications", label: "Certifications", body: "Industry-recognised credentials.", icon: Award },
  { to: "/academy/internships", label: "Internships", body: "Project-based experience.", icon: BookOpen },
  { to: "/academy/admissions", label: "Admissions", body: "Apply and book counselling.", icon: GraduationCap },
...
export function AcademyMegaMenu() {
  const [open, setOpen] = useState<null | "programs" | "resources" | "campuses">(null);
  ...
  const toggle = (panel: "programs" | "resources" | "campuses") => {
    setOpen((v) => {
  ...
  return (
    <div ref={wrapRef} className="relative">
      <ul className="flex items-center gap-1 text-sm font-medium text-muted-foreground whitespace-nowrap">
        <li>
          <MegaTrigger label="Programs" expanded={open === "programs"} onClick={() => toggle("programs")} />
        </li>
        <li>
          <MegaTrigger label="Resources" expanded={open === "resources"} onClick={() => toggle("resources")} />
        </li>
        <li>
          <MegaTrigger label="Campuses" expanded={open === "campuses"} onClick={() => toggle("campuses")} />
        </li>
        <li>
          <Link
      ...
      {open === "programs" && <ProgramsPanel onClose={() => setOpen(null)} />}
      {open === "resources" && <ResourcesPanel onClose={() => setOpen(null)} />}
      {open === "campuses" && <CampusesPanel onClose={() => setOpen(null)} />}
    </div>
  );
}

function MegaTrigger({ label, expanded, onClick }: { label: string; expanded: boolean; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
...
function PanelShell({ children, onClose, wide }: { children: React.ReactNode; onClose: () => void; wide?: boolean }) {
  return (
    <div
      role="dialog"
...
function ProgramsPanel({ onClose }: { onClose: () => void }) {
  ...
  return (
    <PanelShell onClose={onClose} wide>
      <div className="grid grid-cols-12 gap-0">
        ...
        <aside className="col-span-3 bg-academy/5 p-6 border-l border-border flex flex-col">
          <div className="text-[11px] font-semibold uppercase tracking-widest text-academy">Get started</div>
...
function CampusesPanel({ onClose }: { onClose: () => void }) {
  return (
    <PanelShell onClose={onClose}>
      <div className="p-6 grid grid-cols-2 gap-3">
        {CAMPUSES.map((c) => (
