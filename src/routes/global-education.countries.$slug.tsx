...
import {
  buildCountryJsonLd,
  buildCountryBreadcrumbJsonLd,
  buildCountryFaqJsonLd,
} from "@/components/site/CountryJsonLd";
...
export const Route = createFileRoute("/global-education/countries/$slug")({
  head: ({ params }) => {
    ...
    if (countryJsonLd)
      scripts.push({ type: "application/ld+json", children: JSON.stringify(countryJsonLd) });
    const breadcrumbJsonLd = buildCountryBreadcrumbJsonLd(slug);
    if (breadcrumbJsonLd)
      scripts.push({ type: "application/ld+json", children: JSON.stringify(breadcrumbJsonLd) });
    ...
    return {
      meta: [
        ...
        {
          property: "og:url",
          content: "https://www.higaet.com/global-education/countries/" + slug,
        },
    ...
    return (
      <SiteShell>
        <PageHero
          ...
          subtitle="The study destination you are looking for is not available."
        />
        <Section className="!pt-0">
          <p className="text-muted-foreground">
            The study destination you are looking for is not available.
          </p>
          <Link
      ...
      <div className="px-6 pt-8">
        <nav aria-label="Breadcrumb" className="max-w-7xl mx-auto">
          <ol className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <li aria-hidden>/</li>
            <li>
              <Link to="/global-education" className="hover:text-ink transition-colors">
                Global Education Hub
              </Link>
            </li>
            <li aria-hidden>/</li>
            <li>
              <Link to="/global-education/countries" className="hover:text-ink transition-colors">
                Countries
              </Link>
            </li>
            <li aria-hidden>/</li>
      ...
      >
        <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground">
          ...
          <span className="inline-flex items-center gap-1.5">
            <BookOpen className="size-3.5" /> {country.primaryLanguage}
          </span>
          <span className="inline-flex items-center gap-1.5">
            <Users className="size-3.5" /> Top {country.topUniversities.length} universities
          </span>
        </div>
      </PageHero>
        ...
        <div className="grid gap-12 lg:grid-cols-[0.7fr_0.3fr]">
          <div className="space-y-8">
            <div className="prose prose-lg max-w-none">
              ...
              <h2 className="font-display text-2xl font-medium text-ink mt-10 mb-4">
              ...
              <ul className="list-disc pl-6 space-y-2 text-muted-foreground">
                ...
                <li>
                  Affordable education" + (country.avgTuitionUsd === 0 ? " (no tuition fees at
                  public universities)" : " with average tuition of $" +
                  String(country.avgTuitionUsd).replace(/\B(?=(\d{3})+(?!\d))/g, ",") + "/year") + "
                </li>
              ...
              <dl className="grid grid-cols-2 gap-4 text-sm text-muted-foreground border-t border-border pt-4">
                <dt className="text-ink/60">Avg Annual Tuition</dt>
                <dd className="text-ink">
                  {country.avgTuitionUsd === 0
                    ? "No tuition fees (public universities)"
                    : "$" +
                      String(country.avgTuitionUsd).replace(/\B(?=(\d{3})+(?!\d))/g, ",") +
                      "/year"}
                </dd>
                <dt className="text-ink/60">Estimated Living Costs</dt>
                <dd className="text-ink">
                  $" + String(country.costOfLivingUsd).replace(/\B(?=(\d{3})+(?!\d))/g, ",") +
                  "/year
                </dd>
                <dt className="text-ink/60">Primary Language</dt>
                <dd className="text-ink">{country.primaryLanguage}</dd>
                <dt className="text-ink/60">Popular Intakes</dt>
                <dd className="text-ink">{country.popularIntakes.join(", ")}</dd>
              </dl>
              ...
              <div className="flex flex-wrap gap-2">
                {country.popularIntakes.map((i, idx) => (
                  <span
                    key={idx}
                    className="inline-flex items-center gap-1.5 px-3 py-1 text-xs font-medium rounded-full bg-global/10 text-global"
                  >
        ...
        <aside className="lg:sticky lg:top-24 space-y-6">
          <div className="rounded-2xl bg-card p-6 ring-1 ring-border">
            ...
            <dl className="space-y-3 text-sm">
              ...
              <div className="flex justify-between">
                <dt className="text-muted-foreground">Living Costs</dt>
                <dd className="text-ink">
                  $" + String(country.costOfLivingUsd).replace(/\B(?=(\d{3})+(?!\d))/g, ",") + "/yr
                </dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-muted-foreground">Post-Study Work</dt>
                <dd className="text-ink">{country.postStudyWork}</dd>
              </div>
              <div className="flex justify-between">
          ...
          <div className="rounded-2xl bg-global/5 p-6 ring-1 ring-global/20">
            <h3 className="font-display text-base font-medium text-ink mb-3">
              Plan your study in " + country.name + "
            </h3>
            <p className="text-sm text-muted-foreground mb-4">
              Our counsellors help you shortlist universities, prepare applications, and navigate
              visa requirements.
            </p>
            <Link
              to="/global-education/contact"
              className="inline-flex items-center gap-1.5 text-sm font-medium text-ink hover:text-global"
            >
