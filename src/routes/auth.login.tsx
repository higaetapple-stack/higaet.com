...
export const Route = createFileRoute("/auth/login")({
  ...
  head: () => ({
    meta: [{ title: "Sign in — HIGAET" }, { name: "robots", content: "noindex" }],
  }),
  component: LoginPage,
});
...
function LoginPage() {
  ...
  async function signInWithGoogle() {
    ...
    try {
      ...
      const redirectTo = window.location.origin + (safeNext ?? "/dashboard");
  ...
  return (
    <AuthCard
      ...
      footer={
        <>
          New to HIGAET?{" "}
          <Link to="/auth/register" search={{ next: search.next }} className="text-ink underline">
            Create an account
          </Link>
        </>
      }
    >
      ...
      {/* Apple Sign In temporarily disabled — provider not configured in Supabase.
          Re-enable by adding Apple provider in Supabase Auth > Providers and uncommenting below.
          Requires: Apple Services ID, Team ID, Key ID, and Private Key configured in Supabase. */}
      <div className="hidden">
        <button
          type="button"
          disabled={true}
          className="w-full inline-flex justify-center items-center gap-2 ring-1 ring-border bg-ink text-surface text-sm font-medium px-4 py-2.5 rounded-md hover:bg-muted transition-colors opacity-40 cursor-not-allowed"
        >
          <AppleMark />
...
function GoogleMark() {
  return (
    <svg className="size-4" viewBox="0 0 48 48" aria-hidden>
      <path
        fill="#FFC107"
        d="M43.6 20.5H42V20H24v8h11.3c-1.6 4.6-6 8-11.3 8a12 12 0 1 1 0-24c3 0 5.8 1.1 7.9 3l5.7-5.7A20 20 0 1 0 44 24c0-1.2-.1-2.4-.4-3.5z"
      />
      <path
        fill="#FF3D00"
      ...
      <path
        fill="#4CAF50"
        d="M24 44c5.2 0 10-2 13.6-5.2l-6.3-5.3A12 12 0 0 1 12.7 28L6.1 33A20 20 0 0 0 24 44z"
      />
      <path
        fill="#1976D2"
        d="M43.6 20.5H42V20H24v8h11.3a12 12 0 0 1-4.1 5.5l6.3 5.3C41.1 35.6 44 30.3 44 24c0-1.2-.1-2.4-.4-3.5z"
      />
    </svg>
  );
}
