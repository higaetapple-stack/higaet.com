...
import { verifyCertificate } from "@/lib/academic.functions";
import { ShieldCheck, ShieldX, ShieldAlert } from "lucide-react";

export const Route = createFileRoute("/verify-certificate/$id")({
  head: ({ params }) => ({
    meta: [
      { title: `Verify Certificate ${params.id} — HIGAET` },
...
function VerifyPage() {
  ...
  const q = useQuery({ queryKey: ["verify-cert", id], queryFn: () => fn({ data: { number: id } }) });
      ...
      <div className="w-full max-w-xl rounded-2xl bg-card ring-1 ring-border p-8">
        ...
        {!q.isLoading && q.data && !q.data.valid && (
          <div className="mt-8 text-center">
            ...
            <h1 className="font-display text-2xl text-ink mt-4">Not found</h1>
